...
amf = np.loadtxt(r'C:\Users\lme19\Documents\Masters\Validation\IO_1km_full.amf') 


# SCDs, uncertainties and ancillary data (m)
date, tmp, sza, balloon, scd, scderr = np.loadtxt(r'C:\Users\lme19\Documents\Masters\Validation\IO_1km_full.scd',skiprows=2, unpack=True, delimiter = ' ') 
# Translate time string into decimal hour [UTC]


df_scd = pd.read_csv(r'C:\Users\lme19\Documents\Masters\Validation\IO_1km_full.scd', skiprows = 2, delimiter = ' ', dtype = str,  names = ['Date', 'Time/UTC', 'SZA/DEG', 'BALLOON_HEIGHT/KM', 'SCD/MOLEC_CM2', 'SCDERR/MOLEC_CM2'])
...
seg = np.genfromtxt(r'C:\Users\lme19\Documents\Masters\Validation\IO_1km_full.seg', delimiter = ' ') 
...
plt.errorbar(scd_air[1000:-80], scd[1000:-80], yerr = scderr[1000:-80], label = 'IO dSCD', capsize = 1, linewidth = 0.1, marker = '.')
plt.errorbar(scd_air[2623:3112], scd[2623:3112], yerr = scderr[2623:3112], label = 'IO dSCD for Regression', capsize = 1, linewidth = 0.1, marker = '.')

plt.ylabel('dSCD in Molecules/cm2')
plt.xlabel('Air Mass in Molecules/cm2')
plt.legend()
plt.title('Langley Plot for IO')
plt.grid()
plt.savefig('Langley_plot_IO', dpi = 400)
plt.show()
