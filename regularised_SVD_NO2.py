...
# AMF matrix (mxn)
amf = np.genfromtxt(r'C:\Users\lme19\Documents\Masters\Validation\NO2_1km_SZA_85_95.amf') 
...
# SCDs, uncertainties and ancillary data (m)
date, tmp, sza, balloon, scd, scderr = np.loadtxt(r'C:\Users\lme19\Documents\Masters\Validation\NO2_1km_SZA_85_95.scd',skiprows=2, unpack=True, delimiter = ' ') 
Teilchendichte_cm3, Fehler, mitt_luftdichte_cm3, MischVerh, Fehler, von_km, bis_km =  np.loadtxt(r'C:\Users\lme19\Documents\Masters\Validation\NO2_1km_SZA_85_95.vd',skiprows=2, unpack =True) 
...
tmp_clean = tmp
sza_clean = sza
scderr_clean = np.sqrt(scderr**2 + (23426971250284.223)**2)
...
df_scd = pd.read_csv(r'C:\Users\lme19\Documents\Masters\Validation\NO2_1km_SZA_85_95.scd', skiprows = 2, delimiter = ' ', dtype = str,  names = ['Date', 'Time/UTC', 'SZA/DEG', 'BALLOON_HEIGHT/KM', 'SCD/MOLEC_CM2', 'SCDERR/MOLEC_CM2'])
...
seg = np.genfromtxt(r'C:\Users\lme19\Documents\Masters\Validation\NO2_1km_SZA_85_95.seg', delimiter = ' ') 
# Generate mid-point layers (n)
height = np.array([seg[i]/2.+seg[i+1]/2 for i in range(len(seg)-1)]) # Height layers (mid-of-levels)
...
ax1.set_xlabel(r'NO$_2$ / molec/cm$^2$', fontsize=fs)
...
ax1.set_xlabel(r'NO$_2$ / molec/cm$^3$', fontsize=fs)
ax1.set_ylabel('Height / km', fontsize=fs);
ax1.legend();
...
plt.savefig('NO2_profile_with_kernel')
...
plt.savefig('NO2_profile_with_kernel_zoomed')
...
ax1.set_xlabel(r'NO2 / Volume Mixing Ratio', fontsize=fs)
ax1.set_ylabel('Height / km', fontsize=fs);
ax1.legend();
...
plt.savefig('NO2_profile_with_kernel_zoomed_vmr')
